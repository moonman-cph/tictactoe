 ...
 *   - Role CREATE events for all current roles, aligned to their assignee's hire date
 *   - Salary UPDATE events for ~40% of staff twice a year (June & December)
 *   - Role level-change (promotion) events for ~8% of roles per pay review round
...
function generateDemoHistory(orgData, { monthsBack = 18, seed = 42 } = {}) {
  ...
  // ── 2b. Assign role creation dates ────────────────────────────────────────
  //   Each role is created on or just before its assignee's hire date.
  //   Unoccupied roles are spread across the first 8 months of the window.

  const roles       = orgData.roles || [];
  const assignments = orgData.roleAssignments || [];

  const roleEnriched = roles.map(r => {
    const assignment = assignments.find(a => a.roleId === r.id);
    const assignee   = assignment ? enriched.find(p => String(p.id) === String(assignment.personId)) : null;
    let creationDate;
    if (assignee) {
      // Role created 0–7 days before the assignee joined (or same day)
      const offset = randInt(rng, 0, 7) * 24 * 60 * 60 * 1000;
      creationDate = new Date(Math.max(winStart.getTime(), assignee._entryDate.getTime() - offset));
    } else {
      // Unoccupied role: spread across first 8 months
      const eightMonths = new Date(winStart);
      eightMonths.setMonth(eightMonths.getMonth() + 8);
      const span = eightMonths - winStart;
      creationDate = new Date(winStart.getTime() + rng() * span);
    }
    return { ...r, _creationDate: creationDate };
  });
  ...
  // ── 3b. Pre-compute level change history (roll back from current levels) ───
  //   Same pattern as salary: iterate pay review rounds newest-first, pick ~8%
  //   of roles for promotion each round, then reverse to chronological order.

  const LEVEL_NAMES = ['L1','L2','L3','L4','L5','L6','L7','L8'];
  const trackLvl    = {};
  roles.forEach(r => { if (r.level) trackLvl[String(r.id)] = r.level; });

  // levelChanges[roleId] = [ {date, oldLevel, newLevel}, ... ] — forward order
  const levelChangeEvents = {};

  [...payReviewDates].reverse().forEach(reviewDate => {
    // Only consider roles that existed by this review date and are promotable (L1–L7)
    const eligible = roleEnriched.filter(r => {
      const rId = String(r.id);
      const lvl = trackLvl[rId];
      if (!lvl) return false;
      const idx = LEVEL_NAMES.indexOf(lvl);
      return idx > 0 && idx <= 6 && r._creationDate <= reviewDate; // idx > 0: has a prior level
    });
    eligible.forEach(r => {
      if (rng() > 0.08) return; // ~8% promotion rate per round
      const rId    = String(r.id);
      const curLvl = trackLvl[rId];
      const curIdx = LEVEL_NAMES.indexOf(curLvl);
      if (curIdx <= 0) return;
      const prevLvl = LEVEL_NAMES[curIdx - 1];
      if (!levelChangeEvents[rId]) levelChangeEvents[rId] = [];
      levelChangeEvents[rId].push({ date: reviewDate, oldLevel: prevLvl, newLevel: curLvl });
      trackLvl[rId] = prevLvl; // rewind for earlier rounds
    });
  });
  // Reverse to chronological order
  Object.values(levelChangeEvents).forEach(arr => arr.reverse());
  ...
  // ── 4b. Emit role CREATE events ───────────────────────────────────────────
  //   Group by creation date, same batching pattern as person CREATEs.

  const rolesByDay = {};
  roleEnriched.forEach(r => {
    const key = r._creationDate.toISOString().slice(0, 10);
    (rolesByDay[key] = rolesByDay[key] || []).push(r);
  });

  let rolesCreatedCount = 0;
  Object.keys(rolesByDay).sort().forEach(dateKey => {
    const batch   = rolesByDay[dateKey];
    const batchTs = withWorkHour(new Date(dateKey + 'T09:00:00Z'), rng);
    const corrId  = uuid();
    const isBulk  = batch.length > 10;
    const bulkId  = isBulk ? uuid() : null;
    ...
    batch.forEach(r => {
      const ts = new Date(batchTs.getTime() + rng() * 120000).toISOString();
      const obj = Object.fromEntries(
      ...
      entries.push({
        ...
        timestamp: ts,
        actorId: null, actorEmail: null, actorRole: null, actorIp: null, actorUserAgent: null,
        operation: 'CREATE', entityType: 'role',
        entityId: String(r.id), entityLabel: r.title || String(r.id),
        field: null, oldValue: null, newValue: obj,
        changeReason: null, source: 'system', bulkId, isSensitive: false,
      });
      rolesCreatedCount++;
    ...
    if (isBulk) {
      entries.push({
        ...
        entityId: null, entityLabel: null,
        field: null, oldValue: null,
        newValue: { personsCreated: 0, personsUpdated: 0, rolesCreated: batch.length, totalEntries: batch.length },
        changeReason: null, source: 'system', bulkId, isSensitive: false,
      });
    }
  });
  ...
  // ── 5b. Emit role level-change (promotion) events ─────────────────────────
  //   Shared correlationId with the salary review round for the same date.

  let promotionEventCount = 0;
  payReviewDates.forEach(reviewDate => {
    const corrId = uuid();
    roles.forEach(r => {
      const rId = String(r.id);
      const evt = (levelChangeEvents[rId] || []).find(e => e.date.getTime() === reviewDate.getTime());
      if (!evt) return;
      const ts = new Date(reviewDate.getTime() + rng() * 7200000).toISOString();
      entries.push({
        id: uuid(), orgId: 'default', correlationId: corrId,
        timestamp: ts,
        actorId: null, actorEmail: null, actorRole: null, actorIp: null, actorUserAgent: null,
        operation: 'UPDATE', entityType: 'role',
        entityId: rId, entityLabel: r.title || rId,
        field: 'level', oldValue: evt.oldLevel, newValue: evt.newLevel,
        changeReason: 'Annual level review', source: 'system', bulkId: null, isSensitive: false,
      });
      promotionEventCount++;
    });
  });
  ...
  const summary = {
    ...
    promotionEvents:         promotionEventCount,
